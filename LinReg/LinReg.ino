...
long int gap = 900000;                      // Time Period Between Samples taking              : 15 min
const int SAMPLE_SIZE = 600;                // Number of Samples used to calculate a and b     : 600 time
float hum_samples[SAMPLE_SIZE];
float temp_samples[SAMPLE_SIZE];
float humtemp_samples[SAMPLE_SIZE];
float cubehum_samples[SAMPLE_SIZE];
float cubetemp_samples[SAMPLE_SIZE];
  ...
  if (prd == 0) {
    for (int i = 0; i < SAMPLE_SIZE ; i++) {
      ...
      if (chk == -1) {
        ...
        hum_samples[i] = humidity;                        //x
        temp_samples[i] = temperature;                    //y
